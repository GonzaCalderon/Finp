...
function FilterSheet({
                         open, onClose, filters, onChange, onClear, typeOptions, categoryOptions, accountOptions, activeCount,
                     }: {
    open: boolean
    onClose: () => void
    filters: Filters
            ...
            {open && (
                <>
                    ...
                    <motion.div
                        ...
                        style={{ background: 'var(--card)', border: '0.5px solid var(--border)', paddingBottom: 'max(24px, env(safe-area-inset-bottom))' }}
                    >
                            ...
                            <div className="space-y-2">
                                ...
                                <div className="flex flex-wrap gap-2">
                                    {[{ value: '', label: 'Todas' }, ...categoryOptions].map((opt) => (
                                        <button key={opt.value} onClick={() => onChange('categoryId', opt.value)}
                                                className="px-3 py-1.5 rounded-lg text-xs font-medium transition-colors"
                                                style={{
                            ...
                            <div className="space-y-2">
                                ...
                                <div className="flex flex-wrap gap-2">
                                    {[{ value: '', label: 'Todas' }, ...accountOptions].map((opt) => (
                                        <button key={opt.value} onClick={() => onChange('accountId', opt.value)}
                                                className="px-3 py-1.5 rounded-lg text-xs font-medium transition-colors"
                                                style={{
...
export default function TransactionsPage() {
    ...
    const {
        ...
        hasMore, total, loadMore, createTransaction, updateTransaction, deleteTransaction,
    } = useTransactions({
        month,
    ...
    const categoryOptions = useMemo(() => categories.map((c) => ({ value: c._id.toString(), label: c.name })), [categories])
    const accountOptions = useMemo(() => accounts.map((a) => ({ value: a._id.toString(), label: a.name })), [accounts])
    ...
    return (
        <motion.div className="p-4 md:p-6 max-w-4xl mx-auto space-y-4" {...fadeIn}>
            ...
            <div className="hidden md:flex items-center gap-2 flex-wrap">
                <FilterChip label="Tipo" active={!!filters.type} options={typeOptions}
                            value={filters.type} onChange={(v) => setFilter('type', v)} />
                <FilterChip label="Categoría" active={!!filters.categoryId} options={categoryOptions}
                            value={filters.categoryId} onChange={(v) => setFilter('categoryId', v)} />
                <FilterChip label="Cuenta" active={!!filters.accountId} options={accountOptions}
                            value={filters.accountId} onChange={(v) => setFilter('accountId', v)} />
                <FilterChip label="Ordenar" active={filters.sort !== 'date_desc'} options={SORT_OPTIONS}
                            value={filters.sort} onChange={(v) => setFilter('sort', v)} />
                {activeFilterCount > 0 && (
                                    ...
                                    return (
                                        ...
                                        >
                                            <div className="py-3 px-4 flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                                                ...
                                                {/* Mobile — fila superior: badge + monto */}
                                                    ...
                                                    <p className="font-semibold tabular-nums text-sm"
                                                       style={{
                                                           color: transaction.type === 'income' ? '#10B981'
                                                               : transaction.type === 'expense' ? 'var(--destructive)'
                                                                   : 'var(--foreground)',
                                                       }}>
                            ...
                            {hasMore && (
                                <div className="pt-2 flex justify-center">
                                    <Button variant="outline" size="sm" onClick={loadMore} disabled={loadingMore}>
                                        {loadingMore ? (
                                            <span className="flex items-center gap-2"><Spinner /> Cargando...</span>
                                        ) : (

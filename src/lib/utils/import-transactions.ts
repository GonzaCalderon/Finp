...
export const IMPORT_TYPE_ALIASES: Record<string, string> = {
    ...
    'tarjeta de credito pago': 'credit_card_payment',
...
export function typeSupportsCategory(type?: string | null): boolean {
    ...
    return ['income', 'expense', 'credit_card_expense'].includes(normalized ?? '')
}

export function typeRequiresCategory(type?: string | null): boolean {
    return typeSupportsCategory(type)
}

export function typeRequiresSourceAccount(type?: string | null): boolean {
    const normalized = normalizeImportTransactionType(type)
    return ['expense', 'credit_card_expense', 'transfer', 'credit_card_payment', 'adjustment'].includes(
        normalized ?? ''
    )
}
...
export function typeRequiresDestinationAccount(type?: string | null): boolean {
    const normalized = normalizeImportTransactionType(type)
    return ['income', 'transfer', 'credit_card_payment'].includes(normalized ?? '')
}

export function typeSupportsDestinationAccount(type?: string | null): boolean {
    return typeRequiresDestinationAccount(type)
}

export function typeSupportsInstallments(type?: string | null): boolean {
    return normalizeImportTransactionType(type) === 'credit_card_expense'
}
...
export function getImportAccountFieldLabel(type?: string | null): string {
    ...
    if (normalized === 'credit_card_payment') return 'Cuenta origen'
    if (normalized === 'income') return 'Cuenta destino'
    return 'Cuenta'
}
...
export function shouldShowFirstClosingMonth(data: ImportParsedData): boolean {
    const normalizedType = normalizeResolvedImportTransactionType(data)
    return normalizedType === 'credit_card_expense' && (data.installmentCount ?? 1) > 1
}
...
export function resolveImportReferences(data: ImportParsedData, accounts: IAccount[], categories: ICategory[]): ImportParsedData {
    const next: ImportParsedData = { ...data }
    const normalizedType = normalizeImportTransactionType(next.type)

    const accountByName = new Map(
        accounts.map((account) => [normalizeFreeText(account.name), account])
    )
    const categoryByName = new Map(
        categories.map((category) => [normalizeFreeText(category.name), category])
    )
    ...
    if (!next.destinationAccountId && next.destinationAccountName) {
        const account = accountByName.get(normalizeFreeText(next.destinationAccountName))
        if (account) next.destinationAccountId = String(account._id)
    }
    ...
    next.type = normalizeResolvedImportTransactionType({
        ...next,
        type: normalizedType,
    })
...
export function normalizeResolvedImportTransactionType(data: ImportParsedData): string | undefined {
    ...
    if (normalizedType === 'expense' && data.sourceAccountId) {
...
export function evaluateImportRow({
    ...
    if (normalizedData.ignored) {
        return {
            ...
            errors,
            warnings,
            status: IMPORT_ROW_STATUS.IGNORED,
        }
    }
    ...
    if (!normalizedType) {
        errors.push(
            'El tipo es requerido. Valores válidos: gasto, ingreso, gasto con TC, pago de tarjeta, transferencia, ajuste.'
        )
    }
    ...
    if (normalizedData.amount === undefined || normalizedData.amount === null || Number.isNaN(normalizedData.amount)) {
        ...
        errors.push('El monto del ajuste debe ser distinto de cero.')
    }
    ...
    if (normalizedData.installmentCount !== undefined && normalizedData.installmentCount < 1) {
        errors.push('La cantidad de cuotas debe ser 1 o mayor.')
    }

    if (normalizedData.installmentNumber !== undefined && normalizedData.installmentNumber < 1) {
        errors.push('La cuota actual debe ser 1 o mayor.')
    }

    if (
        normalizedData.installmentCount &&
        normalizedData.installmentNumber &&
        normalizedData.installmentNumber > normalizedData.installmentCount
    ) {
        errors.push(
            `Cuota actual (${normalizedData.installmentNumber}) no puede ser mayor que cuotas totales (${normalizedData.installmentCount}).`
        )
    }

    if (normalizedData.categoryName && !normalizedData.categoryId && typeSupportsCategory(normalizedType)) {
        warnings.push(`Categoría "${normalizedData.categoryName}" no encontrada en Finp.`)
    }

    if (normalizedData.accountName && !normalizedData.sourceAccountId && typeRequiresSourceAccount(normalizedType)) {
        errors.push(`Cuenta "${normalizedData.accountName}" no existe en Finp.`)
    }
    ...
    if (
        ...
        errors.push(`Cuenta destino "${normalizedData.destinationAccountName}" no existe en Finp.`)
    }

    if (
        normalizedData.cardName &&
        normalizedType === 'credit_card_expense' &&
        (!sourceAccount || sourceAccount.type !== ACCOUNT_TYPES.CREDIT_CARD)
    ) {
        errors.push(`Tarjeta "${normalizedData.cardName}" no existe en Finp o no es una tarjeta válida.`)
    }

    if (
        normalizedData.cardName &&
        normalizedType === 'credit_card_payment' &&
        (!destinationAccount || destinationAccount.type !== ACCOUNT_TYPES.CREDIT_CARD)
    ) {
        errors.push(`Tarjeta "${normalizedData.cardName}" no existe en Finp o no es una tarjeta válida.`)
    }

    if (typeRequiresCategory(normalizedType)) {
        if (!normalizedData.categoryId) {
            warnings.push('La categoría es obligatoria para este tipo de transacción.')
        } else if (category && categoryType && category.type !== categoryType) {
            errors.push('La categoría elegida no es compatible con el tipo de transacción.')
        }
    }

    if (!typeSupportsCategory(normalizedType) && normalizedData.categoryId) {
        warnings.push('Este tipo no usa categoría. Se ignorará al importar.')
    }

    if (typeRequiresSourceAccount(normalizedType)) {
        if (!normalizedData.sourceAccountId) {
            warnings.push('La cuenta origen es obligatoria para este tipo de transacción.')
        }
    }

    if (typeRequiresDestinationAccount(normalizedType)) {
        if (!normalizedData.destinationAccountId) {
            warnings.push('La cuenta destino es obligatoria para este tipo de transacción.')
        }
    }

    if (normalizedType === 'credit_card_expense') {
        if (sourceAccount && sourceAccount.type !== ACCOUNT_TYPES.CREDIT_CARD) {
            errors.push('Un gasto con TC debe usar una tarjeta de crédito como cuenta.')
        }

        const installmentCount = normalizedData.installmentCount ?? 1
        if (installmentCount > 1 && !normalizedData.firstClosingMonth) {
            warnings.push('Indicá el mes de primera cuota para poder importar el plan.')
        }
    }

    if (normalizedType === 'credit_card_payment') {
        if (
            sourceAccount &&
            ([ACCOUNT_TYPES.CREDIT_CARD, ACCOUNT_TYPES.DEBT] as string[]).includes(sourceAccount.type)
        ) {
            errors.push('El pago de tarjeta debe salir de una cuenta que no sea tarjeta ni deuda.')
        }

        if (
            destinationAccount &&
            !([ACCOUNT_TYPES.CREDIT_CARD, ACCOUNT_TYPES.DEBT] as string[]).includes(destinationAccount.type)
        ) {
            errors.push('El pago de tarjeta debe apuntar a una tarjeta de crédito.')
        }
    }
    ...
    const hasRequiredWarnings =
        (typeRequiresCategory(normalizedType) && !normalizedData.categoryId) ||
        (typeRequiresSourceAccount(normalizedType) && !normalizedData.sourceAccountId) ||
        (typeRequiresDestinationAccount(normalizedType) && !normalizedData.destinationAccountId) ||
        (normalizedType === 'credit_card_expense' &&
            (normalizedData.installmentCount ?? 1) > 1 &&
            !normalizedData.firstClosingMonth)

    const status = errors.length > 0
        ? IMPORT_ROW_STATUS.INVALID
        : missingCore || hasRequiredWarnings || warnings.some((warning) => warning.includes('obligatoria'))
